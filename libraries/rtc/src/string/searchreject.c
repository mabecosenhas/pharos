...
size_t rtcStringSearchReject(const char *s , const char *reject)
{
    ...
    /* iterator in "reject" */
    const char *rejectIterator;


    /* search characters in the "s" string */
    for(iterator = s; *iterator != '\0'; iterator++)
    {
        /* check if the character "iterator" is inside the "reject" string */
        for(rejectIterator = reject; (*rejectIterator != '\0') && (*iterator != *rejectIterator); rejectIterator++);

        /* if reached the end of the reject string and have located the iterator */
        if(*rejectIterator != '\0')
        {
            /* then stop looking */
            break;
