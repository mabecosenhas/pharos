...
#ifndef PHAROS_RTCENDIANBIG_H
#define PHAROS_RTCENDIANBIG_H
...
{
    buffer[0] = (uint8_t) ((number >> 56U) & 0xffU);
    buffer[1] = (uint8_t) ((number >> 48U) & 0xffU);
    buffer[2] = (uint8_t) ((number >> 40U) & 0xffU);
    buffer[3] = (uint8_t) ((number >> 32U) & 0xffU);
    buffer[4] = (uint8_t) ((number >> 24U) & 0xffU);
    buffer[5] = (uint8_t) ((number >> 16U) & 0xffU);
    buffer[6] = (uint8_t) ((number >> 8U) & 0xffU);
    buffer[7] = (uint8_t) (number & 0xffU);
}


INLINE uint16_t rtcEndianHost2Be16(uint16_t source)
{
    /* since we are already in big-endian then nothing needs to be done */
    return source;
}
