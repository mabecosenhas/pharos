...
void pharosCpuThreadFinishCall(ptrThreadInfo thread , ptrPharosCpuInterPartitionContext context)
{
    ...
    /* save the original ra (return address) */
    thread->cpuContext.raIpc = thread->cpuContext.baseContext.raContext;

    /* switch the return address to the exit */
    thread->cpuContext.baseContext.raContext = (uintptr_t) & pharosCpuInterParititionContextExit;
}
