...
{
    /* get the start of the interrupt stack for the running core */
    uint8_t *stackStart = pharosCpuInterruptStack[pharosICoreRunning()];


    /* check if the interrupt nest level is not too deep */
    if(pharosCpuSpGet() > stackStart + PHAROS_CPU_INTERRUPT_STACK_MIN_SIZE)
    {
