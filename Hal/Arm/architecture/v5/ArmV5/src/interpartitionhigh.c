...
void pharosCpuThreadFinishCall(ptrThreadInfo thread , ptrPharosCpuInterPartitionContext context)
{
    ...
    thread->cpuContext.r14Ipc = thread->cpuContext.baseContext.r14_supervisor;

    /* switch the return address to the exit */
    thread->cpuContext.baseContext.r14_supervisor = (uintptr_t) & pharosCpuInterParititionContextExit;
}
