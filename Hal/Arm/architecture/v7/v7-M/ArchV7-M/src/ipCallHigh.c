                                       ...
                                       ptrPartitionCpuMmuContext context , void *functionPointer , ptrPartition caller , ptrPartition ipcallOwner)
{
    ...
    /* get the thread partition */
    ptrPartition partition = running->partition;


    PHAROS_ASSERT(pharosCpuControlIsUnpriv() == FALSE , "Running in unprivileged mode");

    /* check that we switching to the thread kernel stack, not to the application stack */
    PHAROS_ASSERT(pharosIStackIsAddressInside(&running->kernelStack , running->cpuContext.currentKernelStack) == TRUE ,
                  "Running in thread %p (thread number = %d , partition %d) inside user stack %p-%p and kernel current stack = %p" ,
                  ...
                  running->cpuContext.currentKernelStack);
