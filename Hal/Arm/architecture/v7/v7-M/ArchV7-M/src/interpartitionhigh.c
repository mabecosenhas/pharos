...
void pharosCpuThreadFinishCall(ptrThreadInfo thread , ptrPharosCpuInterPartitionContext context)
{
    ...
    thread->cpuContext.previousSvcStack = context->previousSvcStack;

    /* save the original r14 (return address) */
    thread->cpuContext.lrIpc = (((uint32_t) thread->cpuContext.baseContext.lr) | 1);

    /* switch the return address to the exit */
    thread->cpuContext.baseContext.lr = (uintptr_t) ((uint32_t) & pharosCpuInterParititionContextExit | 1);

    /* disable the shared stack MPU region (no arguments needed anyway) */
    pharosCpuRemoveSharedStack(thread , NULL);
 
    /* switch partition MPU context to the previous */
    pharosCpuMpuSwitchPartition(&thread->partition->cpuInfo.partitionMpuContext);
}
